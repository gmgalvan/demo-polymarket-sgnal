terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/lv-3-cluster-services/platform-observability/01-monitoring/terraform.tfstate"
    region       = "us-east-1"
    use_lockfile = true
    encrypt      = true
  }
...
data "aws_secretsmanager_secret_version" "grafana_admin_password" {
  secret_id = data.terraform_remote_state.security_and_config.outputs.managed_secret_names["grafana"]
}
