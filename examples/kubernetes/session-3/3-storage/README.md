...
05 PVC + vLLM (EBS)    the model cache survives the Pod (ReadWriteOnce)
        |
06 static PV + PVC    binding rules and reclaim policy
        |
...
```bash
kubectl apply \
  --filename examples/kubernetes/session-3/3-storage/01-emptydir.yaml
...
```bash
kubectl exec --namespace session-3 random-number-generator \
  --container reader -- touch /opt/restart-me

kubectl get pod random-number-generator --namespace session-3
```
...
```bash
kubectl delete pod random-number-generator --namespace session-3
...
```bash
kubectl apply \
  --filename examples/kubernetes/session-3/3-storage/02-hostpath.yaml
...
still there, because `/data-session-3` lives on the node:

```bash
kubectl delete pod hostpath-number-generator --namespace session-3
...
> Between rehearsals, wipe `/data-session-3` on the node or the file keeps
...
```bash
kubectl apply \
  --filename examples/kubernetes/session-3/3-storage/03-dynamic-pvc.yaml

kubectl get pvc,pod --namespace session-3
```

Both stay `Pending`. Look at the reason:

```bash
kubectl describe pvc signal-history --namespace session-3
```
...
```bash
kubectl get pvc,pod --namespace session-3 --watch
```
...
```bash
kubectl exec --namespace session-3 deployment/signal-writer -- \
  cat /data/history.txt

kubectl delete pod --namespace session-3 --selector app=signal-writer

kubectl rollout status deployment/signal-writer --namespace session-3
...
```bash
cat examples/kubernetes/session-3/3-storage/04-storageclass-ebs.yaml

kubectl apply \
  --filename examples/kubernetes/session-3/3-storage/04-storageclass-ebs.yaml
...
```bash
kubectl apply \
  --filename examples/kubernetes/session-3/3-storage/05-pvc-ebs-vllm.yaml

kubectl get pvc model-cache-ebs --namespace session-3
...
```bash
kubectl exec --namespace session-3 deployment/tinyllama-vllm-ebs -- \
  du -sh /models
...
*(Run `kubectl delete pod --namespace session-3 --selector app=tinyllama-vllm-ebs`
...
```bash
kubectl logs --namespace session-3 deployment/tinyllama-vllm-ebs \
...
```bash
cat examples/kubernetes/session-3/3-storage/06-static-pv.yaml

kubectl apply \
  --filename examples/kubernetes/session-3/3-storage/06-static-pv.yaml

kubectl get pv
kubectl get pvc --namespace session-3
```

Expected result:

```text
pv-vol1           1Gi   RWO   Retain   Bound     session-3/myclaim   manual
...
```bash
kubectl describe pvc oversized-claim --namespace session-3 | tail -5
```
...
```bash
kubectl exec --namespace session-3 pv-consumer -- cat /data/claim.txt
```
...
```bash
kubectl delete pod pv-consumer --namespace session-3
kubectl delete pvc myclaim --namespace session-3
...
```bash
kubectl delete pvc oversized-claim --namespace session-3 --ignore-not-found
...
cat examples/kubernetes/session-3/3-storage/07-efs-storageclass.yaml
```
...
```bash
kubectl apply \
  --filename examples/kubernetes/session-3/3-storage/08-efs-model-store.yaml

kubectl get pvc model-store-efs --namespace session-3
```
...
```bash
kubectl get pods --namespace session-3 --selector job-name=efs-model-downloader --output wide

kubectl logs --namespace session-3 job/efs-model-downloader --follow
...
```bash
kubectl apply \
  --filename examples/kubernetes/session-3/3-storage/09-efs-vllm.yaml
...
```bash
kubectl logs --namespace session-3 deployment/tinyllama-vllm-efs | head -40
```
...
kubectl logs --namespace session-3 efs-reader

kubectl exec --namespace session-3 efs-reader -- \
...
```bash
kubectl exec --namespace session-3 deployment/tinyllama-vllm-efs -- \
...
```bash
kubectl describe pvc <name> --namespace session-3
```
...
```bash
kubectl describe pod <name> --namespace session-3
...
```bash
kubectl describe pod --namespace session-3 --selector app=tinyllama-vllm-efs
```
...
```bash
kubectl run hostpath-cleanup -n session-3 --rm -i --restart=Never --image=busybox:1.36 \
  --overrides='{"spec":{"nodeSelector":{"workload":"core"},"containers":[{"name":"c","image":"busybox:1.36","command":["sh","-c","rm -rf /h1/* /h2/*; echo clean"],"volumeMounts":[{"name":"h1","mountPath":"/h1"},{"name":"h2","mountPath":"/h2"}]}],"volumes":[{"name":"h1","hostPath":{"path":"/data-session-3","type":"DirectoryOrCreate"}},{"name":"h2","hostPath":{"path":"/mnt/pv-session-3","type":"DirectoryOrCreate"}}]}}'
```
