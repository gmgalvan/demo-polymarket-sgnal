...
from the Chapter 8 reference, read [`CONCEPTOS.md`](./CONCEPTOS.md).
...
```bash
kubectl apply \
  --filename examples/kubernetes/sesion-3/4-networking/01-backends.yaml

kubectl apply \
  --filename examples/kubernetes/sesion-3/4-networking/03-client.yaml
...
```bash
kubectl apply \
  --filename examples/kubernetes/sesion-3/4-networking/02-services.yaml
...
kubectl exec \
  --namespace sesion-3 \
  network-client -- \
  wget -qO- http://model-api.sesion-3.svc.cluster.local/
```
...
```text
http://k8s.demo.gmgalvan.com/model/*
  -> Service model-api:80 -> model-api Pods

http://k8s.demo.gmgalvan.com/signal/*
...
```bash
kubectl apply \
  --filename examples/kubernetes/sesion-3/4-networking/04-ingress-alb.yaml \
  --dry-run=server \
  --output yaml
```
...
- The host rule accepts requests for `k8s.demo.gmgalvan.com`.
...
curl --header 'Host: k8s.demo.gmgalvan.com' \
  "http://${ALB_ADDRESS}/model/"
```

After the test succeeds, create a Route 53 Alias record named
`k8s.demo.gmgalvan.com` pointing to the ALB. No record currently exists with
that name in the public `gmgalvan.com` hosted zone.

This first version uses HTTP. No ACM certificate covering `gmgalvan.com` exists
...
Browser -> Route 53 -> ALB -> Ingress -> signal-chat-ui Service
                                      -> Nginx frontend Pods
                                      -> tinyllama-vllm.sesion-3.svc.cluster.local
...
```bash
kubectl apply \
  --filename examples/kubernetes/sesion-3/4-networking/05-ai-chat.yaml
