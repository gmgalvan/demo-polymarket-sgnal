...
```bash
cat examples/kubernetes/sesion-3/2-secrets/demo-api-key.txt
```
...
kubectl create secret generic model-api-credentials \
  --namespace sesion-3 \
  --from-literal=API_USERNAME=demo-user \
  --from-file=API_KEY=examples/kubernetes/sesion-3/2-secrets/demo-api-key.txt
```
...
```bash
cat examples/kubernetes/sesion-3/2-secrets/01-secret.yaml

kubectl apply \
  --filename examples/kubernetes/sesion-3/2-secrets/01-secret.yaml
```
